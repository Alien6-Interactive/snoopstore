<?php
		...
		while ( !feof($contenu) )# tant que je n'est pas atteinds la fin du fichier $fileName.
		{
			...
			if ( $i != 0 )# si ce n'est pas le nom des colonnes de mon fichier csv.
			{
					...
					{	
						...
						else if ( $cle == 5 or $cle == 8 or $cle == 10 or $cle == 12)
						{
							
							$val =  substr($valeur,1,-1);
							if ($val == "")# si $valeur est null.
							{
								
								$donnees [] = "2018/01/14" ; # on ajoute à $donnees la valeur 0 par défaut.
							}
							else
							{
	...
	*/
	...
	CreateDataBase("mises_a_jours");
	ouvrirRepertoire("MiseAJours/");#on appelle la fonction ouvrirRepertoire.
